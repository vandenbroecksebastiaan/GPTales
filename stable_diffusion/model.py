...
from stable_diffusion.util import (
...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, arch="ViT-H-14", version="laion2b_s32b_b79k",
        ...
        super().__init__()
        assert layer in ["last", "penultimate"]
...
class DDIMSampler(object):
    def __init__(self, model, schedule="linear", device=torch.device("cuda"),
                 **kwargs):
        super().__init__()
        self.model = model
    ...
    def make_ddim_sampling_parameters(self, alphacums, ddim_timesteps, eta, verbose=False):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform",
                      ddim_eta=0., verbose=True):
        self.ddim_timesteps = self.make_ddim_timesteps(
        ...
        ddim_sigmas, ddim_alphas, ddim_alphas_prev = \
                self.make_ddim_sampling_parameters(alphacums=alphas_cumprod.cpu(),
    ...
    def sample(self, S, batch_size, shape, conditioning=None, callback=None,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                ...
                cbs = ctmp.shape[0]
                if cbs != batch_size:
                    print(f"Warning: Got {cbs} conditionings but batch-size is" \
                            " {batch_size}")

            elif isinstance(conditioning, list):
                for ctmp in conditioning:
                    if ctmp.shape[0] != batch_size:
                        print(f"Warning: Got {cbs} conditionings but batch-size" \
                                " is {batch_size}")

            else:
                if conditioning.shape[0] != batch_size:
                    print(f"Warning: Got {conditioning.shape[0]} conditionings" \
        ...
        samples, intermediates = self.ddim_sampling(
            conditioning, size, callback=callback, img_callback=img_callback,
            ...
            corrector_kwargs=corrector_kwargs, x_T=x_T, log_every_t=log_every_t,
    ...
    def ddim_sampling(self, cond, shape, x_T=None, ddim_use_original_steps=False,
        ...
        time_range = reversed(range(0,timesteps)) if ddim_use_original_steps \
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False,
        ...
        alphas = self.model.alphas_cumprod if use_original_steps \
                 else self.ddim_alphas
        alphas_prev = self.model.alphas_cumprod_prev if use_original_steps \
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        self.diffusion_model = UNetModel(**config["model"]["params"]["unet_config"]["params"])
        self.conditioning_key = conditioning_key
        assert self.conditioning_key in [None, 'concat', 'crossattn', 'hybrid',
                                         'adm', 'hybrid-adm', 'crossattn-adm']
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self, unet_config, timesteps=1000, beta_schedule="linear",
                 loss_type="l2", ckpt_path=None, ignore_keys=[],
                 ...
                 original_elbo_weight=0., v_posterior=0., l_simple_weight=1.,
                 conditioning_key=None, parameterization="eps",
                 scheduler_config=None, use_positional_encodings=False,
                 learn_logvar=False, logvar_init=0., make_it_fit=False,
                 ucg_training=None, reset_ema=False, reset_num_ema_updates=False,):
        super().__init__()
        ...
        if ckpt_path is not None:
            ...
            if reset_ema:
                assert self.use_ema
                print(f"Resetting ema to pure model weights. This is useful" \
                       " when restoring from an ema-only checkpoint.")
                self.model_ema = LitEma(self.model)
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear",
        ...
        if exists(given_betas):
            ...
            betas = self.make_beta_schedule(beta_schedule, timesteps,
...
class UNetModel(torch.nn.Module):
    ...
    def __init__(
        ...
        if legacy:
            #num_heads = 1
            dim_head = ch // num_heads if use_spatial_transformer \
...
class LatentDiffusion(DDPM):
    def __init__(self, first_stage_config, cond_stage_config, num_timesteps_cond=None,
                 ...
                 cond_stage_forward=None, conditioning_key=None, scale_factor=1.0,
                 scale_by_std=False, force_null_conditioning=False, *args, **kwargs):
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        and batch_idx == 0 and not self.restarted_from_ckpt:
    ...
    def instantiate_first_stage(self, config):
        ...
        model = AutoencoderKL(**config["params"])
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                ...
                # model = instantiate_from_config(config)
                model = FrozenOpenCLIPEmbedder(**config["params"])
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, return_codebook_ids=False,
                        quantize_denoised=False, return_x0=False, score_corrector=None,
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None,
                              quantize_denoised=False, img_callback=None, mask=None,
                              x0=None, temperature=1., noise_dropout=0.,
        ...
        for i in iterator:
            ...
            img, x0_partial = self.p_sample(
                img, cond, ts, clip_denoised=self.clip_denoised,
...
def generate(prompt, model, sampler, steps, path):
    with torch.no_grad(), autocast("cuda"), model.ema_scope():
        c = model.get_learned_conditioning(prompt)
        samples, _ = sampler.sample(
            S=steps, conditioning=c, batch_size=1, shape=[4, 64, 64],
            verbose=False, unconditional_guidance_scale=9,
            unconditional_conditioning=model.get_learned_conditioning([""]),
            eta=0.0, x_T=None
        )
        x_samples = model.decode_first_stage(samples)
        x_samples = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0)
        x_sample = x_samples.squeeze()
